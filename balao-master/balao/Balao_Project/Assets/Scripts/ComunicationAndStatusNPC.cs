	...
	void Update () {
		...
		if ((!start_dial) && (comunication) && (interaction) && (!psd)) {
			if (!acpt){
				...
				obj_0.GetComponent<SpriteRenderer>().sortingOrder = - 1;
		...
		if (start_dial) {
			...
			if(tempo_intervalo < Time.time - tempo_criaçao &&!(create[0])){
				...
				obj_1.transform.position = new Vector2 (symbol_position.transform.position.x + (0.7f * transform.localScale.x),symbol_position.transform.position.y + 0.4f);
			...
			if(tempo_intervalo * 2 < Time.time - tempo_criaçao &&!(create[1])){
				...
				obj_2.transform.position = new Vector2 (symbol_position.transform.position.x - (0.9f * transform.localScale.x),symbol_position.transform.position.y + 0.4f);
				if (transform.localScale.x > 0){
			...
			if(tempo_intervalo * 3 < Time.time - tempo_criaçao &&!(create[2])){
				...
				obj_3.transform.position = new Vector2 (symbol_position.transform.position.x - (0.1f * transform.localScale.x), symbol_position.transform.position.y + 0.4f);
				if (transform.localScale.x > 0){

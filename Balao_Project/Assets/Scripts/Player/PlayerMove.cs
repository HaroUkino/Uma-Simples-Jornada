...
public class PlayerMove : MonoBehaviour {
	...
	private float maxH;
	private float[] timer = new float[] {0,0,0,0};
	...
	void Update () {
		...
		if ((PlayerPrefs.GetFloat("R") == 0) && (PlayerPrefs.GetFloat("G") == 0) && (PlayerPrefs.GetFloat("B") == 0)){
			PlayerPrefs.SetFloat("R", 1f);
			PlayerPrefs.SetFloat("G", 1f);
			PlayerPrefs.SetFloat("B", 1f);
		}
	...
	void FixedUpdate(){
		if ((flying) && (timer[3] < max_height)){
			rigidbody2D.AddForce (new Vector2 (0,jmp_speed));
			fly_audio = true;
			timer[3] = Mathf.Lerp(timer[3],max_height,Time.deltaTime * 5);
			if (timer[3] > max_height - 0.001f){
				timer[3] = max_height;
			}
		} else {
			fall = true;

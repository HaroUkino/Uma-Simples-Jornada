...
public class ComunicationAndStatusNPC : MonoBehaviour {
	...
	public AudioClip[] fx = new AudioClip[] {};
	public bool turn;
	public bool acptin;
	...
	void Update () {
		...
		if (!plr_mov.cinematic) {
			interaction = (Input.GetKeyDown (KeyCode.C));
		} else {
			interaction = plr_bln.Comunicate;
		}
		if ((!start_dial) && (comunication) && (interaction) && (!psd)) {
			if (!acpt){

				if (PlayerPrefs.GetInt("Fdial") == 0){
					PlayerPrefs.SetInt("Fdial", 1);
				}
				...
				if (!plr_mov.cinematic) {
					if (transform.position.x > plr_mov.transform.position.x){
						plr_mov.transform.localScale = new Vector2 (1f,transform.localScale.y);
					}
					if (transform.position.x < plr_mov.transform.position.x){
						plr_mov.transform.localScale = new Vector2 (-1f,transform.localScale.y);
					}
				...
				obj_0.GetComponent<SpriteRenderer>().sortingOrder = 4;
				...
				if (!acptin){
					...
					obj_4.GetComponent<SpriteRenderer>().sortingOrder = 5;
		...
		if (start_dial) {
			...
			if(tempo_intervalo < Time.time - tempo_criaçao &&!(create[0])){
				...
				obj_1.GetComponent<SpriteRenderer>().sortingOrder = 5;
				obj_1.transform.parent = transform;
				obj_1.transform.position = new Vector2 (symbol_position.transform.position.x - (-0.8f * transform.localScale.x),symbol_position.transform.position.y);
			...
			if(tempo_intervalo * 2 < Time.time - tempo_criaçao &&!(create[1])){
				...
				obj_2.GetComponent<SpriteRenderer>().sortingOrder = 5;
				obj_2.transform.parent = transform;
				obj_2.transform.position = new Vector2 (symbol_position.transform.position.x - (1f * transform.localScale.x),symbol_position.transform.position.y);
				if (transform.localScale.x > 0){
			...
			if(tempo_intervalo * 3 < Time.time - tempo_criaçao &&!(create[2])){
				...
				obj_3.GetComponent<SpriteRenderer>().sortingOrder = 5;
				obj_3.transform.parent = transform;
				obj_3.transform.position = new Vector2 (symbol_position.transform.position.x - (0.1f * transform.localScale.x), symbol_position.transform.position.y + 0.6f);
				if (transform.localScale.x > 0){
			...
			if(tempo_intervalo * 4 < Time.time - tempo_criaçao &&!(create[3])){
				...
				if (acpt){
					dread_jdg = judge[3];
					AudioSource.PlayClipAtPoint(fx[3],transform.position,0.5f);
					GameObject.FindGameObjectWithTag ("Player").GetComponent<Animator>().SetBool ("happy", true);
				} else {
					dread_jdg = judge[2];
					int f = Random.Range (0,2);
					AudioSource.PlayClipAtPoint(fx[f],transform.position,0.5f);
					GameObject.FindGameObjectWithTag ("Player").GetComponent<Animator>().SetBool ("sad", true);
				}
				if ((stats_value - plr_bln.stats_value == 1) && (change)){ // Se For (Igual) a '1',Balao Do NPC e o Proximo Da Sequencia - 
